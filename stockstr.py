...
title = "Tnstc(kum)ltd., Trichy Region - Computer Stock Details       Date:- "
footer = "\n\n\n    Asst.                   Supdt.                    B.M."
...
def update(Id):
    ...
    if result:
        ...
        st.write(tabulate(table, headers=headers, tablefmt="grid"))
...
def delete(Id):
    ...
    if result:
        ...
        table = PrettyTable(headers)
        table.add_row(result)
        st.write(tabulate(table, headers=headers))
...
menu_choice = st.sidebar.selectbox("Select an Option", ["Home", "Add New Data", "Update Existing Data", "Delete Data", "Search Data", "View Data"])
...
if menu_choice == "Home":
    ...
    if st.button("Search"):
        ...
        if table_data:
            st.write(tabulate(table_data, headers=headers, tablefmt="grid"))
